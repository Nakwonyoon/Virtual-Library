...
module.exports = function(app) {
  ...
  app.get("/goodReads/:term", function(req, res) {
    request(
      `https://www.goodreads.com/search.xml?key=kfqIZ6fbDX5FN0hEnk62w&q=${req.params.term}`,
  ...
  app.get("/goodReads/book/:author/:id", function(req, res) {
    request(
      `https://www.goodreads.com/author/list/${req.params.author}?format=xml&key=kfqIZ6fbDX5FN0hEnk62w`,
      function(error, response, body) {
        if (error) {
          console.log("There was an Error.");
        } else {
          parseString(body, function(err, result) {
            for (
              i = 0;
              i < result.GoodreadsResponse.author[0].books[0].book.length;
              i++
            ) {
              if (
                result.GoodreadsResponse.author[0].books[0].book[i].id[0]._ ==
                req.params.id
              ) {
                console.log();
                res.json(
                  (results = {
                    title:
                      result.GoodreadsResponse.author[0].books[0].book[i].title,
                    name:
                      result.GoodreadsResponse.author[0].books[0].book[i]
                        .authors[0].author[0].name,
                    pub_date:
                      result.GoodreadsResponse.author[0].books[0].book[i]
                        .publication_year,
                    description:
